  ...
  return (
    <div className="min-h-screen py-8 px-4" style={{ background: brand.colors.background }}>
      <div className="max-w-5xl mx-auto">
        ...
        {/* Filters */}
        <div className="rounded-2xl shadow-xl p-6 mb-6" style={{ background: brand.colors.surface, border: `1px solid ${brand.colors.primarySoft}` }}>
          <div className="flex items-center gap-2 mb-3">
            <Filter size={18} strokeWidth={2} style={{ color: brand.colors.textMuted }} />
            <span className="text-sm font-medium" style={{ color: brand.colors.textMuted }}>Filter by</span>
          </div>
          <div className="flex flex-wrap gap-3">
            {[
        ...
        {/* Transactions List */}
        {filteredTransactions.length === 0 ? (
          <div className="rounded-2xl shadow-xl p-12 text-center" style={{ background: brand.colors.surface, border: `1px solid ${brand.colors.primarySoft}` }}>
            <Receipt size={56} strokeWidth={1.2} style={{ color: brand.colors.textMuted, margin: '0 auto 16px' }} />
            <h3 className="text-xl font-semibold" style={{ color: brand.colors.text }}>No Transactions Found</h3>
            <p style={{ color: brand.colors.textLight }}>Start depositing or withdrawing to see your history here.</p>
            <div className="mt-6 flex flex-wrap gap-4 justify-center">
              <button
              ...
              <button
                ...
                style={{ background: brand.colors.surfaceAlt, color: brand.colors.primary, border: `2px solid ${brand.colors.primary}` }}
              >
          ...
          <div className="space-y-3">
            {filteredTransactions.map((transaction) => {
              const TxIcon = getTransactionIcon(transaction.type, transaction.depositType, transaction.withdrawType);
              ...
              const CurrencyIcon = getCryptoIcon(transaction.currency);
              ...
              return (
                ...
                >
                    ...
                    <div className="text-right">
                      <p
                        className={`font-bold text-lg ${
                          isDeposit ? 'text-green-600' : isWithdraw ? 'text-red-600' : isSwap ? 'text-blue-600' : ''
                        }`}
                      >
                      ...
                      <p className="text-sm" style={{ color: brand.colors.textMuted }}>
                        {transaction.depositType || transaction.withdrawType || transaction.type}
                      </p>
                    </div>
                  ...
                  {(transaction.walletAddress || transaction.bankName || transaction.adminNote) && (
                    <div
                      className="mt-3 pt-3 border-t text-sm space-y-1"
                      style={{ borderColor: brand.colors.primarySoft, color: brand.colors.textMuted }}
                    >
                      {transaction.walletAddress && (
                        <p>
                          <span className="font-medium">Wallet:</span> {transaction.walletAddress}
                        </p>
                      )}
                      {transaction.bankName && (
                        <p>
                      ...
                      {transaction.adminNote && (
                        <p>
                          <span className="font-medium">Note:</span> {transaction.adminNote}
                        </p>
                      )}
                    </div>
                  )}
