...
const Deposit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ ...formData, amount: '', transactionId: '', description: '', proofImage: null });
  ...
  const getAvailableCurrencies = () => Object.keys(addresses);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg font-medium transition hover:opacity-80 text-sm sm:text-base w-full sm:w-auto flex items-center justify-center gap-2"
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-6">
          ...
          >
            <Gift size={40} strokeWidth={1.5} className="mx-auto mb-2" />
        ...
        {/* Main Form Card */}
        <div className="rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 mb-6" style={{ background: brand.colors.surface, border: `1px solid ${brand.colors.primarySoft}` }}>
          {error && (
            <div className="border-l-4 px-4 py-3 rounded-lg mb-6 flex items-center gap-3 text-sm sm:text-base" style={{ backgroundColor: '#FDF2F2', borderColor: brand.colors.error, color: brand.colors.error }}>
          ...
          {success && (
            <div className="border-l-4 px-4 py-3 rounded-lg mb-6 flex items-center gap-3 text-sm sm:text-base" style={{ backgroundColor: '#F0FDF4', borderColor: brand.colors.success, color: brand.colors.success }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
            ...
            {depositType === 'crypto' && (
              <div>
                  ...
                  {getAvailableCurrencies().map((currency) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-3 sm:p-4 rounded-xl border-2 transition transform hover:scale-105 flex flex-col items-center ${
                      ...
                      >
                        ...
                        <div className="text-xs mt-1 truncate w-full text-center" style={{ color: brand.colors.textMuted }}>
                ...
                {selectedAddress && (
                  <div className="mt-3 p-3 rounded-lg" style={{ background: brand.colors.surfaceAlt, border: `1px solid ${brand.colors.primarySoft}` }}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                    {giftcardTypes.map((type) => {
                      const GiftIcon = getGiftcardIcon(type);
                      return (
                        <button
                          key={type}
                          type="button"
                          ...
                          className={`p-3 sm:p-4 rounded-xl border-2 transition transform hover:scale-105 flex flex-col items-center ${
                            formData.giftcardType === type ? 'border-4' : 'border-gray-200'
                          }`}
                          style={{
                        ...
                        >
                          <GiftIcon size={28} strokeWidth={1.8} className="mb-1" style={{ color: brand.colors.primary }} />
                    ...
                    {countries.map((country) => (
                      <button
                        ...
                        className={`p-3 sm:p-4 rounded-xl border-2 transition transform hover:scale-105 ${
                          formData.giftcardCountry === country.code ? 'border-4' : 'border-gray-200'
                        }`}
                        style={{
            ...
            {/* Amount Input */}
            <div>
              <label className="block font-semibold mb-2 text-base sm:text-lg flex items-center gap-2" style={{ color: brand.colors.text }}>
            ...
            <div>
              ...
              <div className="border-2 border-dashed rounded-xl p-4 sm:p-6 text-center transition cursor-pointer" style={{ borderColor: brand.colors.primarySoft, background: brand.colors.surfaceAlt }}>
                <input id="fileInput" type="file" accept="image/*" onChange={handleFileChange} required className="hidden" />
                <label htmlFor="fileInput" className="cursor-pointer block">
                  <Upload size={36} strokeWidth={1.5} style={{ color: brand.colors.primary, margin: '0 auto 8px' }} />
        ...
        {recentDeposits.length > 0 && (
          <div className="rounded-2xl shadow-xl p-4 sm:p-6" style={{ background: brand.colors.surface, border: `1px solid ${brand.colors.primarySoft}` }}>
            <h3 className="font-semibold text-base sm:text-lg mb-4 flex items-center gap-2" style={{ color: brand.colors.text }}>
              <Receipt size={20} strokeWidth={2} />
              {t('deposit_history')}
            </h3>
            <div className="space-y-2 sm:space-y-3">
              {recentDeposits.map((deposit) => {
                const HistIcon = deposit.depositType === 'crypto' ? getCryptoIcon(deposit.currency) : Gift;
                return (
                  <div key={deposit._id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 p-3 sm:p-4 rounded-xl transition" style={{ background: brand.colors.surfaceAlt, border: `1px solid ${brand.colors.primarySoft}` }}>
